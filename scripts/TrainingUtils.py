# TrainingUtils.py - OPTIMIZED & FIXED with finish time CSV logging
import numpy as np
import pygame
import math
...
# ============================================================================
# TRAINING STATISTICS - OPTIMIZED WITH FINISH TIME TRACKING
# ============================================================================

def save_training_stats(episode, avg_reward, losses, epsilon, 
                       win_count=0, avg_checkpoints=0, 
                       total_checkpoints=16,
                       best_finish_time=0.0,  # NEW
                       best_finish_episode=0,  # NEW
                       avg_finish_time=0.0,  # NEW
                       filename="training_stats.csv"):
    """
    Optimized stats saving with finish time tracking
    
    Args:
        episode: Current episode number
        avg_reward: Average reward over last 100 episodes
        losses: List of losses for this checkpoint
        epsilon: Current exploration rate
        win_count: Number of wins in last 100 episodes
        avg_checkpoints: Average checkpoints reached in last 100
        total_checkpoints: Total checkpoints in track (16)
        best_finish_time: Best (highest) time remaining at finish (NEW)
        best_finish_episode: Episode where best time was achieved (NEW)
        avg_finish_time: Average time remaining for successful runs in last 100 (NEW)
    ...
    avg_completion_time = max(0.0, 25.0 - avg_finish_time)
    ...
    with open(filename, 'a', newline='') as f:
        ...
        if write_header:
            writer.writerow([
                ...
                'WinRate_%', 
                'Avg_Checkpoints_%',
                'Best_Finish_Time_Remaining',  # NEW
                'Best_Completion_Time',  # NEW
                'Best_Finish_Episode',  # NEW
                'Avg_Finish_Time_Remaining_100',  # NEW
                'Avg_Completion_Time_100',  # NEW
            ])
        ...
        writer.writerow([
            ...
            f"{win_rate:.1f}",
            f"{checkpoint_pct:.1f}",
            f"{best_finish_time:.2f}",  # NEW
            f"{best_completion_time:.2f}",  # NEW
            best_finish_episode,  # NEW
            f"{avg_finish_time:.2f}",  # NEW
            f"{avg_completion_time:.2f}",  # NEW
        ])
