...
class RaceSettingsScreen(MenuScreen):
    ...
    CAR_COLORS = ["Red", "Blue", "Green", "Yellow", "ice"]
    ...
    def _load_car_images(self):
        ...
        for color, path in CAR_COLORS.items():
            ...
            car_images[color] = pygame.transform.scale(scaled, (100, 50))
        return car_images
    ...
    def _create_player_buttons(self, p1_x, p2_x, button_width, top_y, spacing):
        ...
        for i, text in enumerate(["Human", "DQN"]):
            ...
            btn1 = self.create_button(
                text, 
                lambda t=text: self._toggle_player1(t),
                p1_x - button_width // 2, 
                y, 
                button_width
            )
            ...
            btn2 = self.create_button(
                text,
                lambda t=text: self._toggle_player2(t),
                p2_x - button_width // 2,
                y,
                button_width
            )
    ...
    def _create_car_buttons(self, p1_car_x, p2_car_x, car_button_width, top_y, spacing):
        ...
        for i, color in enumerate(self.CAR_COLORS):
            y = top_y + (i + 1) * spacing
    ...
    def _create_action_buttons(self, center_x, screen_w, screen_h):
        ...
        # Start button - changed text from "Start Race" to "Start"
        start_y = int(screen_h * 0.85)
        self.create_button(
            "Start",  # Changed from "Start Race"
            self._start_race,
            center_x - 150,
            start_y,
            300,
    ...
    def _draw_all_buttons(self, surface):
        ...
        for i, button in enumerate(self.player1_buttons):
            is_selected = game_state_manager.player1_selection == ["Human", "DQN"][i]
        ...
        for i, button in enumerate(self.player2_buttons):
            is_selected = game_state_manager.player2_selection == ["Human", "DQN"][i]
            self._draw_custom_button(
                surface, button, is_selected,
